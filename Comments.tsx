...
const Comments = ({ videoId }: any) => {
  ...
  // 👉 Dummy video detect
  const isDummyVideo =
    !videoId || String(videoId).length !== 24;

  // Dummy comments – sirf UI ke liye
  ...
  const handleSubmitComment = async () => {
    ...
    // ✅ Dummy video → sirf local state update
    if (isDummyVideo) {
    ...
    try {
      ...
      if (res.data.comment) {
        // comment backend se bhi aa sakta hai, but UI ke liye hum turant add kar dete hain
        const newCommentObj: Comment = {
  ...
  const handleDelete = async (id: string) => {
    // ✅ Dummy video → sirf local se hata do
    if (isDummyVideo) {
